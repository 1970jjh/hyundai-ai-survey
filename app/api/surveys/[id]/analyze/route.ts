...
import { getSurvey, listResponses, saveInsights } from "@/lib/surveys";
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await analyzeTexts({ apiKey: s.geminiKey, model: s.model }, survey.title, groups);
    const insights = await saveInsights(id, {
      analysis: { data: result.data, analyzed: result.analyzed, total: result.total, createdAt: new Date().toISOString() },
    });
